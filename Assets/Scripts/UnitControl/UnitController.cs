...
public class UnitController : MonoBehaviour {
    ...
    void Start() {
        ...
        if (useColorFader) {
            ...
            if (headModel)
                colorFade.SetHeadRenderer(headModel.GetComponent<Renderer>());
        }
        ...
        if (!gameControl)
            Debug.Log(transform.name + " can't find the game control.");		
        ...
        if (!dummy)
            gameObject.AddComponent<UnitBehaviors>().SetUp(this, mapControl);
    ...
    public void RemoveEnemyCaptain() {
        ...
        if (!ai) {
            Debug.Log(transform.name + " has no enemyAI!");
            return;
        }
    ...
    public void MoveTo(Vector3 location) {
        ...
        if ((location - transform.position).magnitude < mapControl.GetGridSize() / 3)
            return; // quit if the path is too short

        MapControl.MapDataPoint newMapCell = mapControl.GetMapData(location);
		
        if (newMapCell.isCollision)
            return; // dont want to move into collision
    ...
    public void Die(DamageInfo damageInfo) {
        ...
        if (spawner)
            spawner.SpawnedUnitDead(lifeTime);
