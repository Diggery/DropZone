...
public class MapControl : MonoBehaviour {
    ...
    void Start() {
        ...
        mapSize.x = Mathf.Abs(upperLeft.x) + Mathf.Abs(lowerRight.x);
        mapSize.y = Mathf.Abs(upperLeft.y) + Mathf.Abs(lowerRight.y);
				
        //Vector2 MapStartPosition = new Vector2(-1, -1);
        //Vector2 MapEndPosition = new Vector2(mapSize.x + 1, mapSize.y + 1);
			
        //Pathfinder.Instance.SetUp(upperLeft, lowerRight);

        //MarkMapArea(new Vector3(10, 6), 5);
    }
    ...
    public int GetCoverForTarget(Transform attacker, Transform target) {
        MapDataPoint targetCover = GetCoverPoint(target.position);
        if (!targetCover.IsCoverPoint()) return 0;
    ...
    public Vector3 MapPosToNearestCell(Vector3 mapPos) {
        Vector3 pos = new Vector3(Mathf.FloorToInt(mapPos.x / gridSize) + 0.5f, 0, Mathf.FloorToInt(mapPos.z / gridSize) + 0.5f);
        return pos;
    }
    ...
    public MapDataPoint GetCoverPoint(Vector3 mapPos) {
        return GetMapData(mapPos);
    }

	
    List<MapDataPoint> GetCoverPointsInRange(Vector3 mapPos, float searchRange) {
        List<MapDataPoint> coverPointsInRange = new List<MapDataPoint>();
		
        List<MapDataPoint> cellsInRange = GetMapCellsInRange(mapPos, searchRange); 
		
        for (int i = 0; i < cellsInRange.Count; i++) {
            if (cellsInRange[i].IsCoverPoint()) coverPointsInRange.Add(cellsInRange[i]);
        }
    ...
    List<MapDataPoint> GetMapArea(Vector2 coor, int range) {
        ...
        for (int y = startY; y <= endY; ++y) for (int x = startX; x <= endX; ++x) mapDataInRange.Add(mapData[y * Mathf.FloorToInt(mapSize.x) + x]);
    ...
    public void MarkMapArea(Vector2 mapLoc, int range) {
		
        List<MapDataPoint> mapCellsInRange = GetMapArea(mapLoc, range);
    ...
    public bool IsPositionVisible(Vector3 pos1, Vector3 pos2) {
        ...
        bool exists = Array.Exists(
                          ...
                          delegate(Vector2 coor) {
                return coor.Equals(MapPosToCoor(mapData2.mapPos));
            }
                      );	
    ...
    public Vector3 FindBestCover(Vector3 currentPos, float searchRange, string tag) {
        ...
        for (int i = 0; i < cellsInRange.Length; i++) {
            ...
            if (!cellsInRange[i].IsCoverPoint()) continue;
			
            float coverRating = (float)cellsInRange[i].GetCoverRating(targets);
        ...
        for (int i = 0; i < cellsInRange.Length; i++) {
            ...
            foreach (GameObject target in targets) {
                if ((target.transform.position - currentPos).sqrMagnitude < localCheckDistance) {
                    if (cellsInRange[i].IsCorner()) {
                        Vector3[] firingPositions = cellsInRange[i].GetFiringPositions();
    ...
    //            UnitControl friendController = friend.GetComponent<UnitControl>();
    //            Vector3 destination;
    //            if (!friendController) {
    ...
    public Vector3 FindCorner(Vector3 currentPos, float searchRange) {
        ...
        MapDataPoint bestCorner = null;
        float dist = Mathf.Infinity;
        for (int i = 0; i < cellsInRange.Length; i++) {
            if (!cellsInRange[i].IsCoverPoint()) continue;
            if (!cellsInRange[i].IsCorner()) continue;
            float coverDist = (cellsInRange[i].mapPos - currentPos).sqrMagnitude;
            if (coverDist < dist) {
                dist = coverDist;
                bestCorner = cellsInRange[i];
            }
        }
