...
public class PathMover : Pathfinding {
    ...
    public void UpdatePath(Vector3 newDestination) {
        if (gameObject.tag == "Enemy") {
            print("Updating Path TO " + newDestination);
        }
    ...
    public void StartPath(Vector3 newDestination) {
        ...
        if (gameObject.tag == "Enemy") {
            print("Starting Path TO " + newDestination);
    ...
    public List<Vector3> SimplifyPath(List<Vector3> complexPath) {
        ...
        for (int currentNode = 0; currentNode < complexPath.Count; currentNode++) {
            ...
            for (int reverseCount = complexPath.Count - 1; reverseCount > currentNode + 1; reverseCount--) {
                ...
                if (currentNode + 1 < complexPath.Count && !hieghtIsSame)
                    continue;
    ...
    public void FinishPath(Vector3 _finishPos) {
        ...
        if (destinationCell.coverPoint)
            RotateTo(destinationCell.coverPoint.GetCoverHeading());
    }
